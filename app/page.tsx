...
import { Fan, Snowflake, Lightbulb, Plug, Projector, CheckCircle, Loader2, AlertCircle, X, Bell, Clock, ChevronDown, ChevronUp, History } from "lucide-react"
...
function HomeContent() {
  ...
  // DATA LISTS
  const [pendingIssues, setPendingIssues] = useState<string[]>([]) // List of things currently broken
  const [solvedHistory, setSolvedHistory] = useState<any[]>([])    // List of things fixed
  const [isHistoryOpen, setIsHistoryOpen] = useState(false)

  // 1. ON LOAD: Fetch Data & Listen for Updates
  useEffect(() => {
    fetchRoomData()
  ...
  // Fetch both Pending (Active) and Solved (History) issues
  const fetchRoomData = async () => {
    if (roomName === "Unknown Area") return

    // 1. Get Pending Issues (To prevent duplicates)
    ...
    if (pending) {
      setPendingIssues(pending.map(p => p.issue))
    }

    // 2. Get Solved History
    const { data: history } = await supabase
      .from('reports')
      .select('*')
      .eq('room_id', roomName)
      .eq('status', 'Completed')
      .order('solved_at', { ascending: false }) // Newest first
  ...
  const handleRealtimeEvent = (payload: any) => {
    // Refresh lists to stay accurate
    fetchRoomData()

    // If an issue was just marked Completed, show the Alert
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setStatus("success")
      setPendingIssues([...pendingIssues, selectedIssue]) // Optimistically add to pending list
  ...
  // Check if selected issue is already reported
  const isDuplicate = selectedIssue && pendingIssues.includes(selectedIssue)

  return (
    <main className="min-h-screen bg-gray-50 font-sans pb-40">
      ...
      {/* 2. LIVE ALERT: "Just Fixed!" (WHITE BACKGROUND FIX) */}
      {liveUpdate && (
        <div className="fixed top-4 left-4 right-4 z-[100] animate-in slide-in-from-top-5 duration-300">
          <div className="bg-white text-slate-800 border-l-4 border-blue-600 p-4 rounded-xl shadow-2xl flex items-center gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-slate-500 text-sm">Technician just fixed the <strong>{liveUpdate}</strong>.</p>
            </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden mb-6">
          ...
          >
            ...
            <div className="flex items-center gap-2">
              <span className="text-xs bg-gray-100 text-gray-500 px-2 py-1 rounded-full font-bold">{solvedHistory.length}</span>
          ...
          {isHistoryOpen && (
            <div className="max-h-48 overflow-y-auto bg-gray-50 border-t border-gray-100">
              {solvedHistory.length === 0 ? (
                <p className="p-4 text-center text-sm text-gray-400">No repair history yet.</p>
              ) : (
                solvedHistory.map((job) => (
                  <div key={job.id} className="p-3 border-b border-gray-100 flex justify-between items-center last:border-0">
                    <span className="text-gray-700 font-medium ml-2">{job.issue}</span>
                    <span className="text-xs text-gray-400 flex items-center gap-1 bg-white px-2 py-1 rounded border border-gray-200">
                      <Clock className="w-3 h-3" /> {formatDate(job.solved_at)}
                    </span>
                  </div>
                ))
              )}
      ...
      {/* 4. SMART STICKY FOOTER (Duplicate Prevention) */}
      {selectedIssue && (
        <div className="fixed bottom-0 left-0 w-full p-6 bg-white/90 backdrop-blur-md border-t border-gray-200 shadow-[0_-10px_40px_rgba(0,0,0,0.1)] animate-in slide-in-from-bottom-4 z-40">
          
          {isDuplicate ? (
            // ORANGE BANNER: If already reported
            <div className="w-full bg-orange-100 border border-orange-200 text-orange-800 p-4 rounded-xl flex items-center justify-center gap-3 shadow-sm">
              ...
              <div className="text-left">
                ...
                <p className="text-xs opacity-80">Someone already reported this issue.</p>
              </div>
            </div>
          ) : (
            // BLUE BUTTON: If new issue
            <button
