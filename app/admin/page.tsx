...
import { CheckCircle, Clock, Hammer, RefreshCw, BarChart3, MapPin, Search, ChevronDown, ChevronUp, X, Calendar, Lock, LogOut, MessageSquare } from "lucide-react"
...
export default function AdminDashboard() {
  ...
  // --- HELPER FOR "OTHER" ISSUES ---
  // Parses "Other: Window broken" into ["Other", "Window broken"]
  const getIssueDisplay = (issueString: string) => {
    if (issueString.startsWith("Other: ")) {
      return {
        title: "Other Issue",
        detail: issueString.replace("Other: ", ""),
        isOther: true
      }
    }
    return { title: issueString, detail: null, isOther: false }
  }
      ...
      <div className="min-h-screen bg-slate-900 flex items-center justify-center p-6">
        <div className="bg-white w-full max-w-sm p-8 rounded-3xl shadow-2xl text-center animate-in zoom-in-95">
          ...
          <form onSubmit={handleLogin}>
            <input type="password" placeholder="••••" maxLength={4} value={pinInput} onChange={(e) => setPinInput(e.target.value)} className="w-full text-center text-3xl font-bold tracking-[1em] p-4 border-2 border-slate-200 rounded-xl focus:border-blue-500 focus:outline-none mb-6 text-slate-800" />
            <button type="submit" className="w-full bg-slate-900 text-white font-bold py-4 rounded-xl shadow-lg hover:bg-slate-800 active:scale-95 transition-all">Unlock Dashboard</button>
          </form>
        </div>
      </div>
    )
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900 relative">
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
            <div className="bg-slate-900 p-6 flex justify-between items-start">
              <div><h2 className="text-white text-2xl font-bold flex items-center gap-2"><CheckCircle className="text-green-400" /> Completed</h2><p className="text-slate-400 mt-1">{selectedJob.room_id}</p></div>
              <button onClick={() => setSelectedJob(null)} className="text-slate-400 hover:text-white"><X /></button>
            </div>
            <div className="p-6 space-y-6">
              <div>
                <label className="text-xs font-bold text-slate-400 uppercase">Issue</label>
                <p className="text-xl font-bold text-slate-800">{selectedJob.issue.replace("Other: ", "")}</p>
                {selectedJob.issue.startsWith("Other: ") && <span className="text-xs bg-purple-100 text-purple-700 px-2 py-1 rounded font-bold mt-2 inline-block">Custom Request</span>}
              </div>
      ...
      <nav className="bg-slate-900 text-white p-4 shadow-md sticky top-0 z-10">
        <div className="max-w-5xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-2 font-bold text-xl"><Hammer className="text-blue-400" /> Maintenance<span className="text-slate-400">Admin</span></div>
          <div className="flex gap-2">
            ...
            <button onClick={handleLogout} className="p-2 hover:bg-red-500/20 text-red-400 hover:text-red-300 rounded-full transition-colors" title="Logout"><LogOut className="w-5 h-5" /></button>
          </div>
        </div>
      </nav>
        ...
        <div className="mb-10">
          ...
          <div className="grid gap-3">
            {activeJobs.length === 0 ? (
              ...
              activeJobs.map((job) => {
                const { title, detail, isOther } = getIssueDisplay(job.issue)
                return (
                  <div key={job.id} className={`bg-white p-5 rounded-xl shadow-sm border-l-4 flex justify-between items-center group hover:shadow-md transition-all ${isOther ? 'border-l-purple-500' : 'border-l-orange-500'}`}>
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {isOther && <span className="text-[10px] bg-purple-100 text-purple-700 px-2 py-0.5 rounded-full font-bold tracking-wide">CUSTOM</span>}
                      </div>

                      {/* Display Custom Details if they exist */}
                      {isOther && detail && (
                        <p className="text-sm text-slate-600 mt-1 italic flex items-center gap-1">
                          <MessageSquare className="w-3 h-3 text-slate-400" /> "{detail}"
                        </p>
                      )}
              ...
              <div className="space-y-2 max-h-[400px] overflow-y-auto pr-2">
                {filteredHistory.map((job) => (
                  <div key={job.id} onClick={() => setSelectedJob(job)} className="p-4 rounded-lg border border-slate-100 hover:border-blue-300 hover:bg-blue-50 cursor-pointer transition-all flex justify-between items-center group">
                    <div><span className="font-bold text-slate-700">{job.room_id}</span><span className="text-slate-400 mx-2">•</span><span className="text-slate-600">{job.issue.replace("Other: ", "")}</span></div>
