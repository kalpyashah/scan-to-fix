...
import { Monitor, Snowflake, Lightbulb, Plug, CheckCircle, Loader2, AlertCircle, X, Bell, History, PenTool, ChevronRight, MapPin } from "lucide-react"
...
function LabContent() {
  ...
  const roomName = searchParams.get("room") || "Unknown Lab"
  ...
  const fetchRoomData = async () => {
    if (roomName === "Unknown Lab") return
  ...
  return (
    <main className="min-h-screen bg-slate-50 font-sans pb-48 relative overflow-x-hidden">
      ...
      {/* --- COMPACT HEADER --- */}
      <div className="bg-gradient-to-br from-indigo-600 to-purple-600 pb-24 pt-10 px-6 rounded-b-[2.5rem] shadow-xl relative z-10">
        <div className="max-w-md mx-auto flex justify-between items-start">
          <div>
            <div className="flex items-center gap-1.5 text-indigo-200 mb-1">
              ...
              <p className="text-sm font-medium uppercase tracking-wider">Lab Location</p>
            </div>
      ...
      <div className="px-6 -mt-12 relative z-20 space-y-4 max-w-md mx-auto">
        ...
        {recentHistory.length > 0 && (
          ...
          >
            <div className="flex items-center gap-3">
              <div className="bg-indigo-100 text-indigo-600 p-2.5 rounded-full ring-4 ring-white">
        ...
        <div className="bg-white rounded-[2rem] p-6 shadow-xl shadow-slate-200/50 border border-slate-100">
          ...
          <div className="grid grid-cols-2 gap-4">
            <IssueButton icon={<Monitor />} label="Computer" isSelected={selectedIssue === "Computer"} onClick={() => setSelectedIssue("Computer")} />
            ...
            <div className="col-span-2">
              <IssueButton icon={<PenTool />} label="Other Issue" isSelected={selectedIssue === "Other"} onClick={() => setSelectedIssue("Other")} />
            </div>
          </div>
        </div>
      </div>
           ...
           <div className="max-w-md mx-auto bg-white/95 backdrop-blur-md border-t border-slate-200 shadow-[0_-10px_40px_rgba(0,0,0,0.1)] p-6 rounded-t-3xl animate-in slide-in-from-bottom-4">
              {selectedIssue === "Other" && (
                <div className="mb-4">
                  ...
                  <input autoFocus type="text" placeholder="e.g. Mouse not working..." value={customText} onChange={(e) => setCustomText(e.target.value)} className="w-full p-4 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:outline-none text-slate-800 font-medium" />
                </div>
              )}
              {isDuplicate ? (
                ...
                <button onClick={handleSubmit} disabled={status === "submitting" || (selectedIssue === "Other" && customText.length < 3)} className="w-full bg-indigo-600 disabled:bg-slate-200 disabled:text-slate-400 text-white text-lg font-bold py-4 rounded-xl shadow-lg shadow-indigo-600/30 hover:bg-indigo-700 active:scale-95 transition-all flex items-center justify-center gap-2">
...
function IssueButton({ icon, label, isSelected, onClick }: { icon: any, label: string, isSelected: boolean, onClick: () => void }) {
  // Added 'w-full' to button to support the col-span-2 full width layout
  return (
    <button onClick={onClick} className={`w-full flex flex-col items-center justify-center gap-3 p-6 rounded-2xl transition-all duration-200 border ${isSelected ? "bg-indigo-50 border-indigo-500 shadow-inner scale-95 ring-2 ring-indigo-500 ring-offset-2" : "bg-slate-50 border-slate-100 hover:bg-white hover:shadow-md hover:border-slate-200"}`}>
      <div className={`transition-colors [&>svg]:w-8 [&>svg]:h-8 ${isSelected ? "text-indigo-600" : "text-slate-400"}`}>{icon}</div>
      <span className={`font-semibold ${isSelected ? "text-indigo-700" : "text-slate-600"}`}>{label}</span>
    </button>
  )
}
